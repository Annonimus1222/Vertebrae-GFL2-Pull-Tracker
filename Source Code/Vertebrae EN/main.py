...
class GachaTrackerGUI:
    ...
    def create_settings_window(self):
        ...
        # Language
        ttk.Label(app_frame, text="Language (Work in Progress):").grid(row=0, column=0, sticky=tk.W, pady=2)
        ...
        # Theme
        ttk.Label(app_frame, text="Theme (Placeholder):").grid(row=1, column=0, sticky=tk.W, pady=2)
            ...
            try:
                ...
                if ConfigManager.save_config():
                    ...
                    if language_var.get() != self.current_language:
                        ...
                        messagebox.showinfo("Success", "Options saved. Restart the app to apply language changes.")
        ...
        def reset_settings():
            ...
            if messagebox.askyesno("Confirm", "Reset to default values?"):
                default_config = {
                    "version": 1,
                    "settings": {
                        "page_limit": 50,
                ...
                messagebox.showinfo("Success", "Options reset to default values")
                settings_window.destroy()
        
        ttk.Button(button_frame, text="Save", command=save_settings).pack(side=tk.RIGHT, padx=(10, 0))
        ttk.Button(button_frame, text="Default", command=reset_settings).pack(side=tk.RIGHT)
...
• If you play on Haoplay and don't know your exact region, try one by one. You won't have to re-enter your data because of this
• Click \"🚀 Start Import\"

**📥 FIDDLER TUTORIAL:**
Step 1: Download and install Fiddler Classic: https://www.telerik.com/download/fiddler (You can lie on the survey **wink wink**)
Step 2: Run Fiddler. Let it start completely
Step 3: Run Girls Frontline 2, login normally
...
Step 5.5: If you don't find the entry, wait a second. Sometimes it is slow
Step 6: Look for the "Headers" button at the top of the program. Click on it
Step 7: In the list below the toolbar, look for an entry called "Authorization". Right-click, "Copy values only". That is your token. You can paste it into this program, removing the "Authorization:" part and the space. Leave just the long token
...
⚠️ **DISCLAIMER**: This project is not officially affiliated, endorsed, 
sponsored, or authorized by Sunborn Network Technology Co., MICA Team, 
or the "Girl's Frontline 2: Exilium" game. This is a community fan project.
"""
