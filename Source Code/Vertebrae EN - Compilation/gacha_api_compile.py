import requests
import json
import os
import sys
from datetime import datetime
from urllib.parse import urlencode
import urllib3

# Disable SSL warnings
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

# Function to get paths compatible with PyInstaller
def resource_path(relative_path):
    """Get the correct path for files in both development and EXE"""
    try:
        # PyInstaller creates a temporary folder in _MEIPASS
        base_path = sys._MEIPASS
    except Exception:
        base_path = os.path.abspath(".")
    
    return os.path.join(base_path, relative_path)

# Get the directory where this script is located
if getattr(sys, 'frozen', False):
    # If packaged as exe
    BASE_DIR = os.path.dirname(sys.executable)
else:
    # If running as script
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# Use resource_path for data paths
DATA_DIR = resource_path('data')
LOCALIZATIONS_DIR = resource_path('localizations')

# Create folders if they don't exist (only in development, in EXE they already exist)
if not getattr(sys, 'frozen', False):
    os.makedirs(DATA_DIR, exist_ok=True)
    os.makedirs(LOCALIZATIONS_DIR, exist_ok=True)

# SERVER CONFIGURATION - UPDATED
SERVERS = {
    "darkwinter": {
        "name": "Darkwinter (US/EU)",
        "code": "darkwinter",
        "endpoint": "https://gf2-gacha-record-us.sunborngame.com/list",
        "zone_endpoint": "https://gf2-zoneinfo-us.sunborngame.com"
    },
    "haoplay": {
        "name": "Haoplay Asia",
        "code": "haoplay", 
        "endpoint": "https://gf2-gacha-record-asia.haoplay.com/list",
        "zone_endpoint": "https://gf2-zoneinfo-asia.haoplay.com"
    },
    "haoplay-jp": {
        "name": "Haoplay Japan", 
        "code": "haoplay-jp",
        "endpoint": "https://gf2-gacha-record-jp.haoplay.com/list",
        "zone_endpoint": "https://gf2-zoneinfo-jp.haoplay.com"
    },
    "haoplay-kr": {
        "name": "Haoplay Korea",
        "code": "haoplay-kr", 
        "endpoint": "https://gf2-gacha-record-kr.haoplay.com/list",
        "zone_endpoint": "https://gf2-zoneinfo-kr.haoplay.com"
    },
    "haoplay-intl": {
        "name": "Haoplay International",
        "code": "haoplay-intl",
        "endpoint": "https://gf2-gacha-record-intl.haoplay.com/list", 
        "zone_endpoint": "https://gf2-zoneinfo-intl.haoplay.com"
    },
    "cn": {
        "name": "CN Server",
        "code": "cn",
        "endpoint": "https://gf2-gacha-record.sunborngame.com/list",
        "zone_endpoint": "https://gf2-zoneinfo.sunborngame.com"
    }
}

class ConfigManager:
    """Application configuration manager"""
    _config = None
    _config_file = resource_path("config.json")
    
    @classmethod
    def load_config(cls):
        """Load configuration from config.json"""
        if cls._config is None:
            default_config = {
                "version": 1,
                "settings": {
                    "page_limit": 50,
                    "request_timeout": 20,
                    "max_retries": 3,
                    "default_language": "EN",
                    "theme": "system"
                }
            }
            
            try:
                if os.path.exists(cls._config_file):
                    with open(cls._config_file, 'r', encoding='utf-8') as f:
                        cls._config = json.load(f)
                    print(f"âœ… Configuration loaded: {cls._config_file}")
                else:
                    cls._config = default_config
                    cls.save_config()
                    print(f"ðŸ“ Default configuration created: {cls._config_file}")
            except Exception as e:
                print(f"âŒ Error loading configuration: {e}")
                cls._config = default_config
        
        return cls._config
    
    @classmethod
    def save_config(cls):
        """Save current configuration"""
        try:
            with open(cls._config_file, 'w', encoding='utf-8') as f:
                json.dump(cls._config, f, indent=2, ensure_ascii=False)
            return True
        except Exception as e:
            print(f"âŒ Error saving configuration: {e}")
            return False
    
    @classmethod
    def get_setting(cls, key, default=None):
        """Get a configuration value"""
        config = cls.load_config()
        return config['settings'].get(key, default)
    
    @classmethod
    def set_setting(cls, key, value):
        """Set a configuration value"""
        config = cls.load_config()
        config['settings'][key] = value
        cls._config = config
        return cls.save_config()

class DataManager:
    """External data manager - Reads from JSON files"""
    _dolls = None
    _weapons = None
    _mbox = None
    _weapon_banners = None
    _promotional_banners = None
    _localization = None
    _current_language = "EN"
    
    @classmethod
    def load_dolls(cls):
        """Load characters dictionary from data/dolls.json"""
        if cls._dolls is None:
            try:
                dolls_path = resource_path('data/dolls.json')
                with open(dolls_path, 'r', encoding='utf-8') as f:
                    dolls_data = json.load(f)
                    cls._dolls = {int(k): v for k, v in dolls_data.items()}
                print(f"âœ… dolls.json loaded: {len(cls._dolls)} characters")
            except Exception as e:
                print(f"âŒ Error loading dolls.json: {e}")
                cls._dolls = {}
        return cls._dolls
    
    @classmethod
    def load_weapons(cls):
        """Load weapons dictionary from data/weapons.json"""
        if cls._weapons is None:
            try:
                weapons_path = resource_path('data/weapons.json')
                with open(weapons_path, 'r', encoding='utf-8') as f:
                    weapons_data = json.load(f)
                    cls._weapons = {int(k): v for k, v in weapons_data.items()}
                print(f"âœ… weapons.json loaded: {len(cls._weapons)} weapons")
            except Exception as e:
                print(f"âŒ Error loading weapons.json: {e}")
                cls._weapons = {}
        return cls._weapons
    
    @classmethod
    def load_mbox(cls):
        """Load mbox dictionary from data/mbox.json"""
        if cls._mbox is None:
            try:
                mbox_path = resource_path('data/mbox.json')
                with open(mbox_path, 'r', encoding='utf-8') as f:
                    mbox_data = json.load(f)
                    cls._mbox = {int(k): v for k, v in mbox_data.items()}
                print(f"âœ… mbox.json loaded: {len(cls._mbox)} items")
            except Exception as e:
                print(f"âŒ Error loading mbox.json: {e}")
                cls._mbox = {}
        return cls._mbox
    
    @classmethod
    def load_weapon_banners(cls):
        """Load weapon banners dictionary from data/weapon_banners.json"""
        if cls._weapon_banners is None:
            try:
                weapon_banners_path = resource_path('data/weapon_banners.json')
                with open(weapon_banners_path, 'r', encoding='utf-8') as f:
                    weapon_banners_data = json.load(f)
                    cls._weapon_banners = {int(k): v for k, v in weapon_banners_data.items()}
                print(f"âœ… weapon_banners.json loaded: {len(cls._weapon_banners)} weapon banners")
            except Exception as e:
                print(f"âŒ Error loading weapon_banners.json: {e}")
                cls._weapon_banners = {}
        return cls._weapon_banners
    
    @classmethod
    def load_promotional_banners(cls):
        """Load promotional banners dictionary from data/promotional_banners.json"""
        if cls._promotional_banners is None:
            try:
                promotional_banners_path = resource_path('data/promotional_banners.json')
                with open(promotional_banners_path, 'r', encoding='utf-8') as f:
                    promotional_banners_data = json.load(f)
                    cls._promotional_banners = {int(k): v for k, v in promotional_banners_data.items()}
                print(f"âœ… promotional_banners.json loaded: {len(cls._promotional_banners)} promotional banners")
            except Exception as e:
                print(f"âŒ Error loading promotional_banners.json: {e}")
                cls._promotional_banners = {}
        return cls._promotional_banners
    
    @classmethod
    def load_localization(cls, language=None):
        """Load translations for specified language"""
        if language:
            cls._current_language = language
        
        try:
            localization_path = resource_path(f'localizations/localization_{cls._current_language}.json')
            with open(localization_path, 'r', encoding='utf-8') as f:
                cls._localization = json.load(f)
            print(f"âœ… Localization loaded: {cls._current_language}")
        except Exception as e:
            print(f"âŒ Error loading localization_{cls._current_language}.json: {e}")
            # Fallback to English
            if cls._current_language != "EN":
                print("ðŸ”„ Trying to load English as fallback...")
                cls._current_language = "EN"
                cls.load_localization()
            else:
                cls._localization = {}
                print("âŒ Could not load any localization file")
        return cls._localization
    
    @classmethod
    def get_text(cls, key, default=None):
        """Get translated text using dot notation: 'ui.title'"""
        if cls._localization is None:
            cls.load_localization()
        
        # Try to load localization if not loaded
        if cls._localization is None:
            cls.load_localization()
        
        keys = key.split('.')
        value = cls._localization
        try:
            for k in keys:
                value = value[k]
            return value
        except (KeyError, TypeError):
            return default if default is not None else key
    
    @classmethod
    def set_language(cls, language):
        """Change application language"""
        cls._current_language = language
        cls._localization = None  # Force reload
        return cls.load_localization(language)

class LocalizationManager:
    """Localization manager for the application"""
    _current_language = "EN"
    
    @classmethod
    def set_language(cls, language):
        """Change application language"""
        cls._current_language = language
        return DataManager.set_language(language)
    
    @classmethod
    def get_text(cls, key, default=None):
        """Get translated text"""
        return DataManager.get_text(key, default)
    
    @classmethod
    def get_current_language(cls):
        return cls._current_language

# Short alias for easy use
_ = LocalizationManager.get_text

class SimpleGachaBackup:
    def __init__(self):
        self.backup_file = os.path.join(BASE_DIR, "backup.json")
        self.data_manager = DataManager()
        self.init_backup()
    
    def init_backup(self):
        """Initialize backup if it doesn't exist"""
        if not os.path.exists(self.backup_file):
            base_structure = {
                "version": 3,
                "created": datetime.now().isoformat(),
                "last_updated": None,
                "records": []
            }
            self.save_backup(base_structure)
            print("ðŸ“ Backup created: backup.json")
    
    def load_backup(self):
        """Load complete backup"""
        try:
            with open(self.backup_file, 'r', encoding='utf-8') as f:
                return json.load(f)
        except Exception as e:
            print(f"âŒ Error loading backup: {e}")
            return self.init_backup()
    
    def save_backup(self, data):
        """Save backup"""
        try:
            data["last_updated"] = datetime.now().isoformat()
            with open(self.backup_file, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
            return True
        except Exception as e:
            print(f"âŒ Error saving backup: {e}")
            return False
    
    def add_new_records(self, new_records):
        """Add ONLY NEW records comparing with existing backup"""
        backup_data = self.load_backup()
        existing_records = backup_data["records"]
        
        print(f"   ðŸ” Comparing {len(new_records)} new records vs {len(existing_records)} existing...")
        
        # For existing backup, create a map of "multi groups"
        existing_groups = self.group_by_multi(existing_records)
        
        # For new records, also group by multi
        new_groups = self.group_by_multi(new_records)
        
        # Combine: for each multi group, add only what doesn't exist
        combined_records = existing_records.copy()
        added_count = 0
        
        for timestamp, new_multi_group in new_groups.items():
            if timestamp not in existing_groups:
                # Completely new multi - add everything
                combined_records.extend(new_multi_group)
                added_count += len(new_multi_group)
                print(f"      âœ… New multi: {datetime.fromtimestamp(timestamp).strftime('%H:%M')} - {len(new_multi_group)} pulls")
            else:
                # Existing multi - verify item by item
                existing_multi_group = existing_groups[timestamp]
                existing_items = {(r['item'], r['pool_id']) for r in existing_multi_group}
                
                new_items_to_add = []
                for new_record in new_multi_group:
                    item_key = (new_record['item'], new_record['pool_id'])
                    if item_key not in existing_items:
                        new_items_to_add.append(new_record)
                
                if new_items_to_add:
                    combined_records.extend(new_items_to_add)
                    added_count += len(new_items_to_add)
                    print(f"      âž• Existing multi: {datetime.fromtimestamp(timestamp).strftime('%H:%M')} - {len(new_items_to_add)} new items")
        
        # Update backup only if there are changes
        if added_count > 0:
            backup_data["records"] = combined_records
            self.save_backup(backup_data)
        
        return added_count
    
    def group_by_multi(self, records):
        """Group records by timestamp (each group is a multi)"""
        groups = {}
        for record in records:
            timestamp = record['time']
            if timestamp not in groups:
                groups[timestamp] = []
            groups[timestamp].append(record)
        return groups
    
    def get_all_records(self):
        """Get all records from backup"""
        backup_data = self.load_backup()
        return backup_data["records"]
    
    def get_statistics(self):
        """Backup statistics"""
        records = self.get_all_records()
        stats = {
            'total_records': len(records),
            'banners': {},
            'last_update': None,
            'multi_count': 0
        }
        
        if records:
            # Group by timestamp to detect multis
            timestamp_groups = {}
            for record in records:
                timestamp = record['time']
                if timestamp not in timestamp_groups:
                    timestamp_groups[timestamp] = []
                timestamp_groups[timestamp].append(record)
            
            # Count multis (groups with more than 1 record)
            stats['multi_count'] = sum(1 for group in timestamp_groups.values() if len(group) > 1)
            
            # Sort by time
            sorted_records = sorted(records, key=lambda x: x['time'])
            stats['oldest'] = datetime.fromtimestamp(sorted_records[0]['time']).strftime("%Y-%m-%d")
            stats['newest'] = datetime.fromtimestamp(sorted_records[-1]['time']).strftime("%Y-%m-%d")
            stats['last_update'] = datetime.now().strftime("%Y-%m-%d %H:%M")
            
            # Count by banner
            for record in records:
                banner_id = record['pool_id']
                stats['banners'][banner_id] = stats['banners'].get(banner_id, 0) + 1
        
        return stats

def get_all_pages_for_type(token, email, type_id, server_code="darkwinter", progress_callback=None):
    """Get ALL pages for a specific type_id - WITH CONFIGURABLE LIMIT"""
    all_records = []
    next_cursor = None
    page_count = 0
    
    # Get configuration
    page_limit = ConfigManager.get_setting('page_limit', 50)
    request_timeout = ConfigManager.get_setting('request_timeout', 20)
    max_retries = ConfigManager.get_setting('max_retries', 3)
    
    # Determine limit mode
    unlimited_mode = (page_limit == -1)
    
    if unlimited_mode:
        print(f"   ðŸ“¦ Getting type_id {type_id}: NO LIMIT MODE")
    else:
        print(f"   ðŸ“¦ Getting type_id {type_id}: {page_limit} pages max")
    
    # Get endpoint for selected server
    server_config = SERVERS.get(server_code, SERVERS["darkwinter"])
    base_url = server_config["endpoint"]
    
    print(f"   ðŸŒ Server: {server_config['name']}")
    
    while True:
        page_count += 1
        retry_count = 0
        success = False
        
        # Base parameters
        params = {
            'game_channel_id': '5',
            'type_id': type_id, 
            'u': email
        }
        
        # Add cursor if exists
        if next_cursor:
            params['next'] = next_cursor
        
        headers = {
            'authorization': token,
            'user-agent': 'UnityPlayer/2019.4.40f1 (UnityWebRequest/1.0, libcurl/7.80.0-DEV)',
            'accept': '*/*',
            'accept-encoding': 'deflate, gzip',
            'content-type': 'application/x-www-form-urlencoded',
            'x-unity-version': '2019.4.40f1'
        }
        
        payload = {'server': '1'}  # This parameter seems to always be '1'
        
        # Try with retries
        while retry_count <= max_retries and not success:
            try:
                url_with_params = f"{base_url}?{urlencode(params)}"
                
                response = requests.post(
                    url_with_params,
                    headers=headers,
                    data=payload,
                    timeout=request_timeout,
                    verify=False
                )
                
                if response.status_code == 200:
                    data = response.json()
                    
                    if data.get('code') == 0:
                        records = data['data']['list']
                        next_cursor = data['data'].get('next', '')
                        
                        print(f"      âœ… Page {page_count}: {len(records)} pulls")
                        
                        # Progress callback if available
                        if progress_callback:
                            progress_callback(f"Page {page_count}: {len(records)} pulls")
                        
                        # ADD ALL RAW RECORDS WITHOUT PROCESSING
                        all_records.extend(records)
                        success = True
                        
                        # If no more data, exit
                        if not next_cursor:
                            print(f"      ðŸ End of server data")
                            break
                    else:
                        error_msg = f"API Error: {data.get('message')}"
                        print(f"      âŒ {error_msg}")
                        if progress_callback:
                            progress_callback(f"âŒ {error_msg}")
                        if retry_count < max_retries:
                            retry_count += 1
                            print(f"      ðŸ”„ Retrying... ({retry_count}/{max_retries})")
                            continue
                        break
                else:
                    error_msg = f"HTTP Error: {response.status_code}"
                    print(f"      âŒ {error_msg}")
                    if progress_callback:
                        progress_callback(f"âŒ {error_msg}")
                    if retry_count < max_retries:
                        retry_count += 1
                        print(f"      ðŸ”„ Retrying... ({retry_count}/{max_retries})")
                        continue
                    break
                    
            except requests.exceptions.Timeout:
                error_msg = f"Timeout on page {page_count}"
                print(f"      â° {error_msg}")
                if progress_callback:
                    progress_callback(f"â° {error_msg}")
                if retry_count < max_retries:
                    retry_count += 1
                    print(f"      ðŸ”„ Retrying... ({retry_count}/{max_retries})")
                    continue
                break
                
            except Exception as e:
                error_msg = f"Connection error: {e}"
                print(f"      âŒ {error_msg}")
                if progress_callback:
                    progress_callback(f"âŒ {error_msg}")
                if retry_count < max_retries:
                    retry_count += 1
                    print(f"      ðŸ”„ Retrying... ({retry_count}/{max_retries})")
                    continue
                break
        
        # If not successful after retries, exit
        if not success:
            break
            
        # Check configurable page limit (only if not unlimited mode)
        if not unlimited_mode and page_count >= page_limit:
            print(f"      âš ï¸  Configured page limit reached ({page_limit})")
            break
            
        # If no more server data, exit
        if not next_cursor:
            break
    
    print(f"      ðŸ“Š Total type_id {type_id}: {len(all_records)} pulls in {page_count} pages")
    return all_records

def get_banner_name(pool_id):
    """Banner names - IMPROVED VERSION WITH LOCALIZATION"""
    data_manager = DataManager()
    
    # 1. Special banners with specific FIXED IDs - USING LOCALIZATION
    special_banners = {
        130002: _("banners.weapons"), 
        130003: _("banners.characters"),
        130004: _("banners.special"), 
        130005: _("banners.beginner"),
        130008: _("banners.event"),
        1001: _("banners.permanent"),
        99001: _("banners.mystery_box")
    }
    
    # If it's a fixed special banner, return its TRANSLATED name
    if pool_id in special_banners:
        return special_banners[pool_id]
    
    # 2. Check weapon banners (variable IDs)
    weapon_banners = data_manager.load_weapon_banners()
    if pool_id in weapon_banners:
        return _("banners.weapons")  # Always "Weapons"
    
    # 3. Check promotional character banners (variable IDs)
    promotional_banners = data_manager.load_promotional_banners()
    if pool_id in promotional_banners:
        return _("banners.promotional")  # Always "Promotional"
    
    # 4. Any other unrecognized ID is "Promotional" by default
    return _("banners.promotional")

def get_item_name(item_id):
    """Item names using dictionaries from JSON files"""
    data_manager = DataManager()
    dolls = data_manager.load_dolls()
    weapons = data_manager.load_weapons()
    mbox = data_manager.load_mbox()
    
    # First search in characters (DOLLS)
    if item_id in dolls:
        name_key = dolls[item_id]["name_key"]
        rarity = dolls[item_id]["rarity"]
        # Use improved localization system
        item_name = DataManager.get_text(name_key, f"Character {item_id}")
        return item_name, rarity
    
    # Then search in weapons (WEAPONS)
    if item_id in weapons:
        name_key = weapons[item_id]["name_key"]
        rarity = weapons[item_id]["rarity"]
        item_name = DataManager.get_text(name_key, f"Weapon {item_id}")
        return item_name, rarity
    
    # Search in mbox
    if item_id in mbox:
        name_key = mbox[item_id]["name_key"]
        rarity = mbox[item_id]["rarity"]
        item_name = DataManager.get_text(name_key, f"Mbox {item_id}")
        return item_name, rarity
    
    # If not found in any dictionary
    return f"Item {item_id}", 3

def get_item_type(item_id):
    """Determine item type (character, weapon, mbox, other)"""
    data_manager = DataManager()
    dolls = data_manager.load_dolls()
    weapons = data_manager.load_weapons()
    mbox = data_manager.load_mbox()
    
    if item_id in dolls:
        return "character"
    elif item_id in weapons:
        return "weapon"
    elif item_id in mbox:
        return "mbox"
    else:
        return "other"

def get_server_display_name(server_code):
    """Get server display name"""
    server_config = SERVERS.get(server_code, SERVERS["darkwinter"])
    return server_config["name"]

def debug_api_response(response):
    """Complete API response debug - TO SOLVE NoneType ERROR"""
    print(f"ðŸ” DEBUG API RESPONSE:")
    print(f"   Status Code: {response.status_code}")
    print(f"   Headers: {response.headers}")
    print(f"   Content: {response.text[:500]}...")  # First 500 chars
    
    try:
        data = response.json()
        print(f"   JSON Structure: {list(data.keys()) if data else 'EMPTY'}")
        if 'data' in data:
            print(f"   Data keys: {list(data['data'].keys()) if data['data'] else 'EMPTY_DATA'}")
    except Exception as e:
        print(f"   JSON Parse Error: {e}")

def get_complete_gacha_history_simple_backup():
    print("=== ðŸš€ GACHA SCRAPER - FINAL SYSTEM ===")
    print("=== ðŸ’¾ COMPARE WITH BACKUP + ALLOW MULTIS ===\n")
    
    # Show server options
    print("ðŸŒ AVAILABLE SERVERS:")
    for code, config in SERVERS.items():
        print(f"   {code}: {config['name']}")
    
    # Configuration
    token = input("\nPaste your authorization token: ").strip()
    email = input("Enter your email: ").strip()
    server_code = input("Server (darkwinter/haoplay/haoplay-jp/etc): ").strip() or "darkwinter"
    
    # Validate server
    if server_code not in SERVERS:
        print(f"âŒ Server '{server_code}' not valid. Using darkwinter by default.")
        server_code = "darkwinter"
    
    print(f"âœ… Selected server: {SERVERS[server_code]['name']}")
    
    # Initialize backup
    backup = SimpleGachaBackup()
    
    # Show current status
    stats = backup.get_statistics()
    print(f"\nðŸ“Š CURRENT BACKUP STATUS:")
    print(f"   Total pulls: {stats['total_records']}")
    print(f"   Multis detected: {stats['multi_count']}")
    if stats['total_records'] > 0:
        print(f"   Range: {stats['oldest']} - {stats['newest']}")
    
    print(f"\nðŸ“¦ Getting data from server...")
    
    # Get RAW data
    type_ids_to_check = ['1', '3', '4', '5', '8']
    all_new_raw_records = []
    
    for type_id in type_ids_to_check:
        print(f"\nðŸŽ¯ Type_id {type_id}:")
        records = get_all_pages_for_type(token, email, type_id, server_code)
        if records:
            # Check duplicates in this request (for info only)
            unique_timestamps = len({r['time'] for r in records})
            if len(records) > unique_timestamps:
                print(f"   ðŸŽ¯ {len(records) - unique_timestamps} multi duplicates detected")
            
            all_new_raw_records.extend(records)
            print(f"   ðŸ“¥ {len(records)} pulls obtained")
        else:
            print(f"   â„¹ï¸  No data")
    
    # ðŸ”¥ COMPARE AND ADD ONLY WHAT'S NEW
    if all_new_raw_records:
        added_count = backup.add_new_records(all_new_raw_records)
        
        print(f"\n{'='*50}")
        print("ðŸ“Š UPDATE RESULT:")
        print(f"   Server pulls: {len(all_new_raw_records)}")
        print(f"   New pulls added: {added_count}")
        print(f"   Duplicates omitted: {len(all_new_raw_records) - added_count}")
        
        # Show updated statistics
        new_stats = backup.get_statistics()
        print(f"\nðŸ“ˆ UPDATED STATUS:")
        print(f"   Total pulls: {new_stats['total_records']}")
        print(f"   Multis detected: {new_stats['multi_count']}")
        
        # Show distribution
        if new_stats['banners']:
            print(f"\nðŸŽ¯ DISTRIBUTION:")
            for banner_id, count in new_stats['banners'].items():
                banner_name = get_banner_name(banner_id)
                print(f"   {banner_name}: {count} pulls")
        
        # Show latest pulls
        if new_stats['total_records'] > 0:
            all_records = backup.get_all_records()
            recent_records = sorted(all_records, key=lambda x: x['time'], reverse=True)[:15]
            
            print(f"\nðŸ“œ LATEST PULLS:")
            current_time_group = None
            for i, pull in enumerate(recent_records, 1):
                time_str = datetime.fromtimestamp(pull['time']).strftime("%Y-%m-%d %H:%M")
                banner_name = get_banner_name(pull['pool_id'])
                item_name, rarity = get_item_name(pull['item'])
                
                # Group by multi visually
                if current_time_group != pull['time']:
                    if current_time_group is not None:
                        print("   â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€")
                    current_time_group = pull['time']
                    multi_count = len([r for r in recent_records if r['time'] == pull['time']])
                    multi_indicator = " ðŸŽ¯" if multi_count > 1 else ""
                    print(f"   {time_str}{multi_indicator}")
                
                print(f"        {banner_name} â†’ {item_name}")
    
    else:
        print(f"\nâŒ No new data obtained")
    
    print(f"\nðŸ’¾ Backup: {os.path.abspath(backup.backup_file)}")
    print(f"\n{'='*60}")
    input("ðŸŽ‰ Press ENTER to close...")

if __name__ == "__main__":
    get_complete_gacha_history_simple_backup()